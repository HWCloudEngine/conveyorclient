...
"""
Command-line interface to the OpenStack Conveyor API.
"""
...
class ConveyorClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(ConveyorClientArgumentParser, self).__init__(*args, **kwargs)
    ...
    def _get_option_tuples(self, option_string):
        ...
        result = super(ConveyorClientArgumentParser, self)._get_option_tuples(
            option_string)
...
class OpenStackConveyorShell(object):

    def get_base_parser(self):
        parser = ConveyorClientArgumentParser(
            prog='conveyor',
            description=__doc__.strip(),
            epilog='Run "conveyor help SUBCOMMAND" for help on a subcommand.',
            add_help=False,
        ...
        parser.add_argument('--version',
                            action='version',
                            version=conveyorclient.__version__)

        parser.add_argument('--debug',
                            action='store_true',
                            default=utils.env('V2VCLIENT_DEBUG',
                                              default=False),
        ...
        parser.add_argument('--service-type',
                            ...
                            help='Service type. '
                            'For most actions, default is conveyor.')
        parser.add_argument('--service_type',
                            help=argparse.SUPPRESS)

        parser.add_argument('--service-name',
                            metavar='<service-name>',
                            default=utils.env('CONVEYOR_SERVICE_NAME'),
                            help='Service name. '
                            'Default=env[CONVEYOR_SERVICE_NAME].')
        ...
        parser.add_argument('--endpoint-type',
                            ...
                            default=utils.env(
                                'CONVEYOR_ENDPOINT_TYPE',
                                default=DEFAULT_CONVEYOR_ENDPOINT_TYPE),
                            help='Endpoint type, which is publicURL or '
                            'internalURL. '
                            'Default=conveyor env[CONVEYOR_ENDPOINT_TYPE] or '
                            + DEFAULT_CONVEYOR_ENDPOINT_TYPE + '.')

        parser.add_argument('--endpoint_type',
                            help=argparse.SUPPRESS)

        parser.add_argument('--os-conveyor-api-version',
                            metavar='<conveyor-api-ver>',
                            default=utils.env('OS_CONVEYOR_API_VERSION',
                                              default=None),
                            help='CONVEYOR API version. '
                            'Valid values are 1 or 2. '
                            'Default=env[OS_CONVEYOR_API_VERSION].')
        parser.add_argument('--os_conveyor_api_version',
                            help=argparse.SUPPRESS)
        ...
        conveyorclient.auth_plugin.discover_auth_systems()
        conveyorclient.auth_plugin.load_auth_system_opts(parser)
    ...
    def _append_global_identity_args(self, parser):
        ...
        parser.add_argument('--insecure',
                            default=utils.env('CONVEYORCLIENT_INSECURE',
                                              default=False),
                            action='store_true',
                            help=argparse.SUPPRESS)
    ...
    def _discover_extensions(self, version):
        ...
        for name, module in itertools.chain(
            ...
            extension = conveyorclient.extension.Extension(name, module)
    ...
    def _discover_via_python_path(self, version):
        for (module_loader, name, ispkg) in pkgutil.iter_modules():
            if name.endswith('python_conveyorclient_ext'):
    ...
    def main(self, argv):
        ...
        if not options.os_conveyor_api_version:
            # Environment variable OS_CONVEYOR_API_VERSION was
            # not set and '--os-conveyor-api-version' option doesn't
            # specify a value.  Fall back to default.
            options.os_conveyor_api_version = DEFAULT_OS_CONVEYOR_API_VERSION
        ...
        if not endpoint_type:
            endpoint_type = DEFAULT_CONVEYOR_ENDPOINT_TYPE
        ...
        self.cs = client.Client(options.os_conveyor_api_version, os_username,
        ...
        try:
            ...
            if endpoint_api_version != options.os_conveyor_api_version:
                msg = (("OpenStack conveyor API version is set to %s "
